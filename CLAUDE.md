...
A weekly refresh runs in GitHub Actions: `.github/workflows/refresh-data.yml` fires Mondays 09:00 UTC and on `workflow_dispatch`. It runs the same `refresh:gbif` → `build:data` → commit pipeline as `scripts/cron_refresh.sh`, but on Ubuntu runners (no TCC issues). GBIF credentials come from repo secrets `GBIF_USER` / `GBIF_PASSWORD` / `GBIF_NOTIFY_EMAIL`. The legacy `scripts/cron_refresh.sh` + `scripts/com.iddl.indd-dashboard-refresh.plist` are kept in the repo for ad-hoc local runs (`bash scripts/cron_refresh.sh`) but the launchd install was removed — repeatedly failed with macOS TCC denials reading `~/Documents` regardless of Full Disk Access grants.
